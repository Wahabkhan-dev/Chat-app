...
const UserActionsMenu: React.FC<{
  ...
  <DropdownMenu>
    <DropdownMenuTrigger asChild>
      <button className="p-2 hover:bg-muted rounded-xl transition-all tap-small"><MoreVertical className="h-4 w-4 text-muted-foreground" /></button>
    </DropdownMenuTrigger>
    <DropdownMenuContent align="end" className="bg-card border-border min-w-[180px] rounded-xl shadow-xl z-[var(--z-dropdown)]">
      ...
      {user.isActive ? (
        ...
        <>
          <DropdownMenuItem className="gap-2 py-2.5 text-green-600 font-medium cursor-pointer focus:bg-green-500/10" onClick={onReactivate}>
            <CheckCircle className="h-4 w-4" /> Reactivate User
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem className="gap-2 py-2.5 text-destructive font-medium cursor-pointer focus:bg-destructive/10" onClick={onDelete}>
...
const AdminDashboard: React.FC = () => {
  ...
  return (
    <>
    <div className="flex-1 overflow-y-auto bg-background p-4 md:p-8 scrollbar-hide animate-in fade-in duration-500">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold font-headline text-foreground tracking-tight">Workspace Admin</h1>
          ...
          <div className="flex gap-2 md:gap-3 w-full md:w-auto">
            <Button variant="outline" className="flex-1 md:flex-none gap-2 border-border hover:bg-muted rounded-xl font-bold h-11" onClick={() => dispatch({ type: 'OPEN_MODAL', payload: { type: 'createGroup' } })}>
              <Users2 className="h-4 w-4" />
              <span>Create Group</span>
            </Button>
            <Button className="flex-1 md:flex-none gap-2 bg-primary text-white hover:bg-primary/90 rounded-xl font-bold h-11 shadow-lg shadow-primary/20" onClick={() => dispatch({ type: 'OPEN_MODAL', payload: { type: 'createUser' } })}>
                    ...
                    {filteredUsers.map(user => (
                      <tr key={user.id} className={cn('hover:bg-muted/20 transition-all group', !user.isActive && 'bg-muted/10')}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <UserActionsMenu user={user} onView={() => dispatch({ type: 'OPEN_MODAL', payload: { type: 'userProfile', data: { user } } })} onEdit={() => dispatch({ type: 'OPEN_MODAL', payload: { type: 'editUser', data: { user } } })} onChangePassword={() => setChangePasswordTarget(user)} onDeactivate={() => handleDeactivate(user)} onReactivate={() => handleReactivate(user)} onDelete={() => handleDeleteUser(user)} currentUserId={state.currentUser?.id} />
                        </td>
                      </tr>
                    ))}
                  </tbody>
